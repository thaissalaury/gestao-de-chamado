...
namespace GestaoChamados
{
    public class FrmClientes : Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            pnlCard.Controls.AddRange(new Control[] { lblNome, txtNome, lblContato, txtContato, btnAdicionar, btnExcluir, btnLimpar });
        ...
        private void BtnExcluir_Click(object? sender, EventArgs e)
        {
            try
            {
                ...
                if (row == null)
                {
                    MessageBox.Show("Selecione um cliente na lista!", "Validação", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                int id = Convert.ToInt32(row.Cells[0].Value);

                if (MessageBox.Show("Deseja realmente excluir este cliente?", "Confirmar Exclusão", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    service.Excluir(id);
            ...
            {
                MessageBox.Show("Erro ao excluir cliente: " + ex.Message, "Erro do Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
